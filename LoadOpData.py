...
def loadOperatorWeapons(operatorName):
    ...
    for weaponCategoryHtml in loadoutHtml.find_all('div', class_='operator__loadout__category'):
        ...
        for weaponInfoHtml in weaponCategoryHtml.find('div', class_='operator__loadout__category__items'):
            weaponName = weaponInfoHtml.find_next('p').text.replace(' ', '_').lower().strip()

            print(f'Load weapon: {weaponName}')

            if os.path.exists(f'{pathToWeaponCategory}/{weaponName}') == False:
                os.makedirs(f'{pathToWeaponCategory}/{weaponName}')

            weaponImageUrl = weaponInfoHtml.find('img')['src']
            weaponImage = requests.get(weaponImageUrl).content
            with open(f'{pathToWeaponCategory}/{weaponName}/{weaponName}_Image.png', 'wb') as file:
            ...
            if weaponType: 
                with open(f'{pathToWeaponCategory}/{weaponName}/{weaponType}.type', 'wb'): pass
            else: 
                with open(f'{pathToWeaponCategory}/{weaponName}/none.type', 'wb'): pass

            print(f'Weapon {weaponName} loaded')
